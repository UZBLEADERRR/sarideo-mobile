...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    $('#create-form').addEventListener('submit', async event => { event.preventDefault(); const topic = $('#topic').value.trim(); if (!topic) return UI.toast('Avval mavzuni yozing'); const formOptions = { format: $('#format').value, duration: Number($('#duration').value), hero: $('#hero').value, voice: $('#voice').value, visualStyle: $('#visual-style').value, subtitles: $('#subtitles').value, music: $('#music').value }; const button = $('#create-form button[type="submit"]'); button.disabled = true; button.innerHTML = '<span>◌</span> Tayyorlanmoqda…'; try { await UI.createProject(topic, formOptions); $('#topic').value = ''; } catch (error) { UI.toast(error.message || 'Loyiha yaratilmadi'); } finally { button.disabled = false; button.innerHTML = '<span>✦</span> Video yaratishni boshlash'; } });
  });
})();
